...
- **git add**: Tanım. Dosyaları Sahneye Alma.
  ...
  - **Patch Modu**: `git add -p`. Kısmi Sahneye Alma. Örnek.
- **git commit**: Tanım. Değişiklikleri Kaydetme.
  ...
  - **Add + Commit**: `git commit -am "mesaj"`. Örnek.
...
- **git log**: Tanım. Commit Geçmişini Görme.
  ...
  - **`--all`**: Tüm Branch'ler. Örnek.
  - **`--author`**: Yazara Göre Filtre. Örnek.
  - **`--since / --until`**: Tarihe Göre Filtre. Örnek.
  - **`-p`**: Değişiklikleri Göster. Örnek.
  - **`--stat`**: Dosya İstatistikleri. Örnek.
- **git diff**: Tanım. Değişiklikleri Karşılaştırma.
  - **Working Directory vs Staging**: `git diff`. Örnek.
  - **Staging vs Son Commit**: `git diff --staged`. Örnek.
  - **İki Commit Arası**: `git diff commit1 commit2`. Örnek.
  - **İki Branch Arası**: `git diff branch1 branch2`. Örnek.
